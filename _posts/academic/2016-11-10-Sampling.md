...
word2vec: $$ \max_{v_i, v_o} \log \sigma( v_o^T v_i ) + \sum_n \mathbb{E}_{n\sim p(i)}\left[ \log \sigma (- v_n^T v_i) \right] $$
...
im2vec: $$ \max_{\{W\}, (v_o)} \sum_p \mathbb{E}_{p \in \text{tags}_i} \log \sigma( v_p^T g(x,\{W\}) ) + \sum_n \mathbb{E}_{n \sim p(i)} \left[ \log \sigma(-v_n^T g(x,\{W\})) \right] $$
